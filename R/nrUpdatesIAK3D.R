...
#################################################           
#            iSmallV <- Matrix::which((lnvPars < smallV) & (checkHess$invCb > 0))
            iSmallV <- Matrix::which((lnvPars < smallV) & (checkHess > 0))
...
#    iXiCX <- chol2inv(tmp$cholC)
	...
	lndetXiCX <- as.numeric(Matrix::determinant(XiCX , logarithm = TRUE)$modulus)

    T <- iC - iCX %*% iXiCX %*% Matrix::t(iCX)
	Tz <- as.matrix(iCz - iCX %*% betahat)
	zTz <- as.numeric(ziCz - 2 * Matrix::t(betahat) %*% XiCz + Matrix::t(betahat) %*% XiCX %*% betahat)

    TdCAll <- T %*% dCAll
    dCAllTz <- Matrix::t(dCAll) %*% Tz
    zTdCTz <- Matrix::t(Tz) %*% matrix(dCAllTz , nrow = n)
	...
	for (i in 1:length(dC)){
        ...
        trTdC[i] <- sum(Matrix::diag(TdCAll[,iThis]))
 		grad[i] <- 0.5 * (trTdC[i] - zTdCTz[i])
        for (j in i:length(dC)){
            jThis <- ((j - 1) * n + 1):(j * n)
            TdCiTdCjThis <- TdCAll[,iThis] %*% TdCAll[,jThis]
            if (i == j){
                hess[i,j] <- as.numeric(0.5 * (trTdC[i] - sum(Matrix::diag(TdCiTdCjThis)) - zTdCTz[i] + 2 * Matrix::t(W[,p+1]) %*% TdCiTdCjThis %*% Tz))
                fim[i,j] <- as.numeric(0.5 * sum(Matrix::diag(TdCiTdCjThis)))
            }else{
                hess[i,j] <- as.numeric(0.5 * (-sum(Matrix::diag(TdCiTdCjThis)) + 2 * Matrix::t(W[,p+1]) %*% TdCiTdCjThis %*% Tz))
                hess[j,i] <- hess[i,j]
                fim[i,j] <- as.numeric(0.5 * sum(Matrix::diag(TdCiTdCjThis)))
                fim[j,i] <- fim[i,j]
            }
